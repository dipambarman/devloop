        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
                <div class="flex items-center gap-3">
                    <h2 class="text-2xl font-bold tracking-tight text-primary-text">{{ $project->name }}</h2>
                    <x-badge :color="$project->status === 'active' ? 'success' : ($project->status === 'completed' ? 'primary' : 'gray')">
                        {{ ucfirst($project->status) }}
                    </x-badge>
                </div>
                @if($project->github_repo)
                    <a href="https://github.com/{{ $project->github_repo }}" target="_blank" class="text-sm text-secondary-text hover:text-primary mt-1 inline-flex items-center gap-1 transition-colors">
                                ...
                                <div class="flex items-center justify-between group">
                                    ...
                                    @can('manageMembers', $project)
                                        <form action="{{ route('projects.removeMember', [$project, $member]) }}" method="POST" class="opacity-0 group-hover:opacity-100 transition-opacity">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-secondary-text hover:text-red-500 transition-colors p-1" title="Remove Member" onsubmit="return confirm('Remove this member?');">
